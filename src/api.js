...
const groupByFirstLetter = S.groupBy(x => y => S.equals(S.head(x))(S.head(y)));

// [['alf', 'argh'],['bee', 'boo']] -> ['a', 'b']
const getEachFirstLetter = S.mapMaybe(S.pipe([S.mapMaybe(S.head), S.head]));
...
export const getImages = breed =>
  get(getImagesUrlFor(breed))
    ...
    .chain(
      res => (res.status !== "success" ? F.of({ message: [] }) : F.of(res))
    )
    .map(S.prop("message"));
